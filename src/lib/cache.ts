...
import { OWResponse } from '../weather/openweather'
...
export class WCache {
  ...
  private async writeStoreEntry(id: string, value: OWResponse) {
  ...
  public async setEntry(id: string, value: OWResponse) {
    try {
  ...
  public async getEntry(id: string): Promise<OWResponse> {
    try {
  ...
  /**
   * Return the cache id for the supplied position
   * @param position Location coordinates
   * @param precision geohash precision (defualt=5)
   */
  public calcId(position: Position, precision = 5): string {
